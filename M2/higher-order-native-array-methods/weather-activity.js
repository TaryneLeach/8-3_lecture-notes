...
function daysWarmerThan50(forecast) {
    let warmDay = (forecast => forecast.date.highTemp > 50);
    forecast.filter(warmDay);
   // forecast.every(warmDay);
    return warmDay;
}
daysWarmerThan50(warmDay);
/**
...
function tempRanges(forecast) {
    return forecast.filter(date => date.highTemp, date.lowTemp)
}
