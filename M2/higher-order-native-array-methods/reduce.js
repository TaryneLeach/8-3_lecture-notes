...
let voteCount = voters.filter((person) => {
  ...
  /*let totalCount = 0
  totalCount = voteCount.reduce((person, el) => {
    return person + el 
  },0)        what the fuck is going on here lmfao
  return totalCount; */
  let totalCount = 0
...
res.numYoungVotes = arr.reduce((acc,el) =>{
  // call reduce + condtional voted true && age 18-25
  ...
  numMidVotesPeople: arr.reduce((acc,el) => {
    if (el.voted && el.age > 26 && el.age <= 35) {
      return acc + 1
    }
    return acc + 0
  },0)
  numMidsPeople: arr.reduce((acc, el) => {
   if (el.age >= 25 )  {
